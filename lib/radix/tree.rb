...
module Radix
  ...
  class Tree
    ...
    # :nodoc:
    def add(path, payload, node = nil)
      return add_w_2_args(path, payload) if node.nil?
    ...
    # :nodoc:
    def find(path, result = nil, node = nil, first = nil)
      ...
      if first && (path.bytesize == node.key.bytesize && path == node.key) && node.payload?
        ...
        while key_reader.has_next? && path_reader.has_next? &&
          ...
          when '*'
            ...
            name = key_reader.string[key_reader.pos + 1]
            value = path_reader.string[path_reader.pos]
          ...
          when ':'
            ...
            # key size.
            name = key_reader.string[key_reader.pos + 1, key_size - 1]
            value = path_reader.string[path_reader.pos, path_size]
